...
.. note::
    ...
    Idéalement, la position initiale doit être une position jouable : un jeu dont toute partie se termine avant que le premier coup ait été joué n'a qu'un intérêt limité...
...
La valeur matérielle n'a aucune incidence sur le comportement sur le jeu.
...
Chaque famille de rayons utilisée par une pièce possède une **portée**, limitant le nombre maximum de cases que peut parcourir la pièce.
Cette portée est :
...
Un saut ``(a, b)`` autorise les déplacements relatifs obtenus par les combinaisons de signes : ``(±a, ±b)`` (es doublons éventuels sont ignorés).

.. note::

    Les coordonnées ne sont pas interchangeables automatiquement.
    Ainsi, un saut ``(1, 2)`` n'implique pas le saut ``(2, 1)``.
    Si les deux sont souhaités, ils doivent être définis séparément.
...
.. note::
    ...
    Le Roi ayant une valeur infinie, il n'intervient pas dans ce calcul.

13. Rapport avec les échecs classiques
---

Les échecs sont "presque" un exemple de JEG.
En effet, leur structure essentielle peut être capturée par les règles de JEG, mais les quelques règles spéciales ne le sont pas.

En effet, les propriétés suivantes des pièces permettent de reproduire le coeur des échecs :
...
Cependant, les règles supplémentaires ne sont pas couvertes par le JEG :
